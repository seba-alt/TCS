...
Virtualized grid of all experts rendered with react-virtuoso (20 cards/page, infinite scroll), rich expert cards with Framer Motion entry animations, and interactive domain tag pills that update sidebar filters on click. The expert database grows over time — the grid must handle an open-ended dataset with no hardcoded counts. Creating experts, profile modals, and AI co-pilot features are separate phases.
